...
const handle = require('micro/lib/handler')
const logError = require('micro/lib/error')
const parseEndpoint = require('micro/lib/parse-endpoint')
...
if (args['--help']) {
  console.error(chalk`
  {bold.cyan rx-micro} - Reactive HTTP microservices

  {bold USAGE}

      {bold $} {cyan rx-micro} --help
      {bold $} {cyan rx-micro} --version
      {bold $} {cyan rx-micro} [-l {underline listen_uri} [-l ...]] [{underline entry_point.js}]

      By default {cyan rx-micro} will listen on {bold 0.0.0.0:3000} and will look first
  ...
  {bold OPTIONS}
      ...
      -v, --version                       displays the current version of rx-micro
  ...
  {bold ENDPOINTS}
      ...
      Listen endpoints (specified by the {bold --listen} or {bold -l} options above) instruct {cyan rx-micro}
      to listen on one or more interfaces/ports, UNIX domain sockets, or Windows named pipes.

      For TCP (traditional host/port) endpoints:

          {bold $} {cyan rx-micro} -l tcp://{underline hostname}:{underline 1234}

      For UNIX domain socket endpoints:

          {bold $} {cyan rx-micro} -l unix:{underline /path/to/socket.sock}

      For Windows named pipe endpoints:

          {bold $} {cyan rx-micro} -l pipe:\\\\.\\pipe\\{underline PipeName}
`)
  process.exit(2)
}
...
function startEndpoint(module, endpoint) {
  const request$ = serve(module, server => {
    server.listen(...endpoint, () => {
      ...
      if (typeof details === 'string') {
        console.log(`rx-micro: Accepting connections on ${details}`)
      } else if (typeof details === 'object' && details.port) {
        console.log(`rx-micro: Accepting connections on port ${details.port}`)
      } else {
        console.log('rx-micro: Accepting connections')
      }
    })

    server.on('error', err => {
      console.error('rx-micro:', err.stack)
      process.exit(1)
    })
  })
  ...
  request$.subscribe({
    ...
    complete: () => console.log('rx-micro: Server closed.'),
  })
}
...
async function start() {
  ...
  registerShutdown(() =>
    console.log('rx-micro: Gracefully shutting down. Please wait...')
  )
}
